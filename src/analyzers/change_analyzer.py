...
class ChangeAnalyzer:
    ...
    def analyze_scans(self, current_data: dict[str, Any], previous_data: dict[str, Any] | None) -> dict[str, Any]:
        if not previous_data:
            return {'has_previous_data': False, 'message': 'First run - no previous data to compare'}
    ...
    def analyze_credentials(self, current_data: dict[str, Any], previous_data: dict[str, Any] | None) -> dict[str, Any]:
        if not previous_data:
            return {'has_previous_data': False, 'message': 'First run - no previous data to compare'}
        ...
        return {
            ...
            'is_significant_change': abs(change_pct) >= threshold,
            'threshold': threshold,
        }

    def analyze_license(self, current_data: dict[str, Any], previous_data: dict[str, Any] | None) -> dict[str, Any]:
        if not previous_data:
            return {'has_previous_data': False, 'message': 'First run - no previous data to compare'}
        ...
        change_pct = (change_count / prev_licensed * 100) if prev_licensed > 0 else 0
    ...
    def analyze_agents(self, current_data: dict[str, Any], previous_data: dict[str, Any] | None) -> dict[str, Any]:
        if not previous_data:
            return {'has_previous_data': False, 'message': 'First run - no previous data to compare'}
    ...
    def _analyze_distribution_changes(self, current_dist: dict[str, int], prev_dist: dict[str, int]) -> dict[str, list[dict[str, Any]]]:
        ...
        for key in all_keys:
            ...
            if prev_count == 0 and current_count > 0:
                ...
                changes['increased'].append({'name': key, 'current': current_count, 'previous': prev_count, 'change': current_count - prev_count})
            elif current_count < prev_count:
                changes['decreased'].append({'name': key, 'current': current_count, 'previous': prev_count, 'change': prev_count - current_count})

        return changes

    def analyze_scanners(self, current_data: dict[str, Any], previous_data: dict[str, Any] | None) -> dict[str, Any]:
        if not previous_data:
            return {
        ...
        return {
            ...
            'new_problem_scanners': [s for s in current_problems if s['id'] not in prev_problem_ids],
            'recovered_scanners': [s for s in prev_problems if s['id'] not in current_problem_ids],
            'ongoing_problem_scanners': [s for s in current_problems if s['id'] in prev_problem_ids],
        }

    def analyze_connectors(self, current_data: dict[str, Any], previous_data: dict[str, Any] | None) -> dict[str, Any]:
        if not previous_data:
            return {
                'has_previous_data': False,
                'message': 'First run - no previous data to compare',
        ...
        return {
            ...
            'current_problem_count': len(current_problems),
            'previous_problem_count': len(prev_problems),
            'new_problem_connectors': [c for c in current_problems if c['id'] not in prev_problem_ids],
            'recovered_connectors': [c for c in prev_problems if c['id'] not in current_problem_ids],
            'ongoing_problem_connectors': [c for c in current_problems if c['id'] in prev_problem_ids],
        }

    def analyze_users(self, current_data: dict[str, Any], previous_data: dict[str, Any] | None) -> dict[str, Any]:
        if not previous_data:
            return {'has_previous_data': False, 'message': 'First run - no previous data to compare'}
